...
This document describes the standard release process for doris-operator.
...
2. **Docker image** — uses `VERSION` directly:
   `quay.io/zncdatadev/doris-operator:<version>`
...
4. **Helm chart publish** — pushes to
   `quay.io/kubedoopcharts/doris-operator:<version>` (OCI registry)
...
```bash
gh api repos/zncdatadev/doris-operator/git/refs \
  -f ref=refs/heads/release-0.x \
  -f sha=$(gh api repos/zncdatadev/doris-operator/git/ref/heads/main --jq .object.sha)
```
...
- Docker image is available at
  `quay.io/zncdatadev/doris-operator:x.y.z-dev`
- Helm chart is available at
  `quay.io/kubedoopcharts/doris-operator:x.y.z-dev`
...
doris-operator follows [Semantic Versioning](https://semver.org/):
